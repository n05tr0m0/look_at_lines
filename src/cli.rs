...
Sorting is always active. Pick a field flag to choose the sort key.
If no field flag is given, sorting defaults to Name (ascending).
...
If you mix conflicting fields (e.g., -ns), the tool safely falls back to Name sorting.
...
pub struct Cli {
    ...
    /// Works with any export format or standalone (copies plain-text name listing).
    #[arg(long = "copy")]
    pub copy: bool,
